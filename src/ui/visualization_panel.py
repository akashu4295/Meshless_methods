...
def _create_convergence_plot_section(themes: dict):
    ...
    # Create plot
    with dpg.plot(label="Residual", height=CONVERGENCE_PLOT_HEIGHT,
        width=-1, tag="convergence_plot"):
        
        dpg.add_plot_axis(dpg.mvXAxis, label="Timestep",
            tag="x_axis_conv")
        dpg.add_plot_axis(dpg.mvYAxis, label="Total Steady-State Error",
            tag="y_axis_conv", log_scale=True)
        
        dpg.add_line_series([], [], parent="y_axis_conv",
            tag="conv_series", label="Convergence")
...
def _create_plotting_options_section(themes: dict):
    ...
    with dpg.group(horizontal=True):
        ...
        dpg.add_input_text(hint="VTK File", default_value=DEFAULT_VTK,
            tag="contour_vtk_path", width=VTK_PATH_INPUT_WIDTH, show=True)
        dpg.add_button(label="Browse", tag="vtk_browse",
            callback=open_file_dialog_callback,
            user_data="vtk", show=True)
        
        themed_texts("Variable:","label")
        dpg.add_combo(PLOT_VARIABLES, label="##Variable",
            default_value=DEFAULT_PLOT_VARIABLE, tag="contour_var",
            width=COMBO_WIDTH, callback=change_variable_callback)
        themed_texts("Colormap:","label")
        dpg.add_combo(COLORMAPS, label="##Colormap",
            default_value=DEFAULT_COLORMAP, tag="contour_cmap",
            width=COMBO_WIDTH, callback=change_colormap_callback)
        plot_btn = dpg.add_button( label="Plot",
    ...
    dpg.add_file_dialog(directory_selector=False,
        tag="file_dialog_vtk", user_data="contour_vtk_path",
        callback=select_vtk_file_callback, show=False,
